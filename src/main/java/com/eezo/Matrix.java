...
public class Matrix implements Cloneable {
    ...
    public Matrix getClone(){
        Matrix newMatrix = new Matrix(TransData.staticInstance.getMatrixRowsNumber(), TransData.staticInstance.getMatrixColsNumber());
        System.arraycopy(matrix.clone(), 0, newMatrix.matrix, 0, matrix.length);
        return newMatrix;
    }
    ...
    public Matrix assignmentSteppingStone(int lastEmptyCellPosFound) {
        ...
        Cell startCell = matrix[lastEmptyCellPosFound];
        ...
        /** This is need for proper work */
        Matrix newMatrix = new Matrix(TransData.staticInstance.getMatrixRowsNumber(), TransData.staticInstance.getMatrixColsNumber());
        for (int i = 0; i < newMatrix.matrix.length; i++) {
            newMatrix.matrix[i] = matrix[i].clone();
        }
    ...
    public Matrix recalculateValues() {
        ...
        for (int i = 0; i < newMatrix.matrix.length; i++) {
            if (newMatrix.matrix[i].getStatus() == -1 && minValue > newMatrix.matrix[i].getValue()) {
                minValue = newMatrix.matrix[i].getValue();
            }
        }
        for (int i = 0; i < newMatrix.matrix.length; i++) {
            if (newMatrix.matrix[i].getStatus() == -1) {
                newMatrix.matrix[i].setValue(newMatrix.matrix[i].getValue() - minValue);
                continue;
            }
            if (newMatrix.matrix[i].getStatus() == 1) {
                newMatrix.matrix[i].setValue(newMatrix.matrix[i].getValue() + minValue);
